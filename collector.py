...
Token order fix: clobTokenIds[0]=UP, clobTokenIds[1]=DOWN
"""
...
def get_token_ids_for_market(ts_end, interval='5m'):
    """
    Polymarket token order for Up/Down:
    clobTokenIds[0] = UP (Yes) token
    clobTokenIds[1] = DOWN (No) token
    Returns (up_id, down_id)
    """
    slug = f'btc-updown-{interval}-{ts_end}'
    try:
        resp = requests.get(
            GAMMA_API,
            params={'slug': slug},
        ...
        if len(tokens) >= 2:
            # index 0 = UP token, index 1 = DOWN token
            up_id   = tokens[0].get('token_id', tokens[0]) \
                      if isinstance(tokens[0], dict) else tokens[0]
            down_id = tokens[1].get('token_id', tokens[1]) \
...
def get_yes_price(token_id):
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            # Sort to get actual best prices
...
def get_order_book_simple(token_id):
    ...
    Bid = real market price (highest buyer).
    Ask = what sellers are asking (lowest seller).
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            # Fix: Sort Bids highest-to-lowest, Asks lowest-to-highest
            sorted_bids = sorted(bids, key=lambda x: float(x['price']), reverse=True)
            sorted_asks = sorted(asks, key=lambda x: float(x['price']))
            
            bid     = float(sorted_bids[0]['price']) if sorted_bids else 0.0
            ask     = float(sorted_asks[0]['price']) if sorted_asks else 0.0
            
            bid_liq = round(sum(
                float(b['price']) * float(b['size'])
                for b in sorted_bids[:5]), 2)
            ask_liq = round(sum(
                float(a['price']) * float(a['size'])
                for a in sorted_asks[:5]), 2)
    except Exception:
        pass
...
def update_pending_outcome():
    ...
    try:
        ...
        # Fix: pass end time of 15m candle
        prev_m15_end = prev_boundary + CANDLE_END
        prev_up_id, _ = get_token_ids_for_market(prev_m15_end, '15m')
        if not prev_up_id:
            return
...
def process_one_window():
    ...
    print('Reading C1...')
    # Fix: pass end time of C1
    c1_end = boundary_15 + C1_CLOSE
    c1_up_id, _ = get_token_ids_for_market(c1_end, '5m')
    if not c1_up_id:
    ...
    print('Reading C2...')
    # Fix: pass end time of C2
    c2_end = boundary_15 + C2_CLOSE
    c2_up_id, _ = get_token_ids_for_market(c2_end, '5m')
    if not c2_up_id:
    ...
    # ── Get 15-min tokens ─────────────────────────────────────
    # Fix: pass end time of 15m candle
    m15_end = boundary_15 + CANDLE_END
    up_id, down_id = get_token_ids_for_market(m15_end, '15m')
    if not up_id or not down_id:
        print('No 15-min tokens. Skipping.')
        return False
        
    # Fix: Safeguard against late start writing empty files
    secs_now = int(time.time()) - boundary_15
    if secs_now > LOG_END:
        print('Started too late in the window. Skipping logging.')
        return False
    ...
    while True:
        ...
        print(f'  [{secs_into_15}s] '
              f'UP bid={up_bid} ask={up_ask} | '
              f'DOWN bid={down_bid} ask={down_ask}')
